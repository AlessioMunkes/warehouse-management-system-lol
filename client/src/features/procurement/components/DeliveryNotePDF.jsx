...
import { fmtQty, variance } from '../../receipts/components/noteFormat';
...
const DeliveryNotePDF = ({ delivery, onClose }) => {
  ...
  // When the lines come off the purchase order rather than the delivery
  // note itself, there is no record of what physically arrived, so the
  // received and variance columns would be empty theatre. Ordered-only,
  // exactly as Serena's original note rendered it.
              ...
              {delivery.items?.length ? (
                delivery.items.flatMap((item, i) => {
                  ...
                  return [
                    ...
                    // The reason sits on its own line rather than in a column,
                    // so it has room to be a sentence instead of a clipped cell.
                    v && item.discrepancy_reason ? (
                      <tr key={`${key}-reason`} className="pdf-reason-row">
                        <td colSpan={fromPO ? 4 : 6}>
                          <strong>Why:</strong> {item.discrepancy_reason}
                        </td>
                      </tr>
                    ) : null,
                  ];
