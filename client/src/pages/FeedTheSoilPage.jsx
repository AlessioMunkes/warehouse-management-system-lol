...
//
// Feed the Soil kit logging — the real, staff-facing screen the
// Impact Calculator's compost_processed metric reads from. Per the
// project's own visit notes: "Feed the soil - take food waste, turn
// to soil. goes to farmers, then we purchase (swapping food waste for
// compost - managing buckets)". A kit is one bucket's round trip:
// logged when it goes out with food waste, logged again when it
// comes back with compost.
//
// MOVED HERE FROM A DIALOG ON THE IMPACT CALCULATOR PAGE.
// Logging a kit is a warehouse-floor action a worker performs when a
// bucket physically leaves or returns, same as a community request or
// a donation intake — it belongs with those, not buried in a
// manager-facing reporting screen. compost_processed still just reads
// the same collection_kits table; nothing about the metric changed.
//
// Construction mirrors CommunityRequestsPage.jsx: ManagerLayout shell,
// a header with a one-line description, a search-free filter +
// primary-button toolbar (there is no free-text search here — a
// warehouse has a handful of kits in flight, not hundreds), the shared
// brand error banner, a Card-wrapped Table, and inline Card forms for
// log / return rather than dialogs. No new visual patterns.
// ─────────────────────────────────────────────────────────────
