...
describe('runEmailReminderJob with several warehouses', () => {
  ...
  it('runs once inside each warehouse, and keeps going when one fails', async () => {
    const { currentWarehouse } = await import('../src/config/warehouseContext.js');
    const seen = [];
    const service = {
      sendTomorrowCollectionReminderEmails: async () => {
        const code = currentWarehouse();
        seen.push(code);
        if (code === 'gauteng') throw new Error('gmail down');
        return { collectionDate: '2026-09-26', queued: {}, attempted: 1, sent: 1, failed: 0, skipped: 0 };
      },
    };
    await expect(runEmailReminderJob({ logger: quiet, service, codes: ['cpt', 'gauteng', 'nc'] }))
      .rejects.toThrow('gmail down');
    expect(seen).toEqual(['cpt', 'gauteng', 'nc']);
  });

  it('is a single plain run with one database', async () => {
    const service = { sendTomorrowCollectionReminderEmails: async () => ({ sent: 2 }) };
